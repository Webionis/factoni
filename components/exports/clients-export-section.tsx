...
export function ClientsExportSection() {
  const [format, setFormat] = useState<ExportFormat>("csv");
  const { downloading, progressLabel, download } = useExportDownload();

  const downloadUrl = useMemo(() => {
    const params = new URLSearchParams({ format });
    return `/api/exports/clients?${params.toString()}`;
  }, [format]);

  return (
    <section className={cn(surfaceCardClassName, "p-6 sm:p-7")}>
      <div className="flex items-start gap-3">
        <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#eff6ff] text-[#2563eb] dark:bg-[rgba(37,99,235,0.15)]">
          <Users className="size-5" aria-hidden />
        </div>
        <div>
          <h2 className={sectionHeadingClassName}>Exporter les clients</h2>
          <p className={cn("mt-1 text-sm leading-relaxed", formSectionDescriptionClassName)}>
            Nom, coordonnées, nombre de factures, CA généré et dernière activité.
          </p>
        </div>
      </div>
      ...
      <Button
        ...
        onClick={() => void download(downloadUrl, "Export clients téléchargé")}
