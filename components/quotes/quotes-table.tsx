...
import { quoteDisplayNumber, type QuoteStatus } from "@/lib/quotes/status";
import {
  ...
  return (
    <div className={dataTableScrollWrapperClassName}>
      <table className={dataTableElementClassName}>
        ...
        <tbody>
          {quotes.map((quote) => {
            if (!isQuoteDocument(quote.document_type)) return null;
            ...
            const number = quoteDisplayNumber(quote.invoice_number, quote.id);

            return (
              <tr key={quote.id} className={dataTableRowClassName}>
