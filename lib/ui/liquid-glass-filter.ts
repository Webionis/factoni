/**
 * Génération du filtre SVG liquid glass (réfraction physique).
 * Porté depuis archisvaze/liquid-glass (index.html).
 */

export type LiquidGlassSurface = "convex_squircle" | "convex_circle" | "concave" | "lip";

export type LiquidGlassNavOptions = {
...
export const DEFAULT_NAV_GLASS_OPTIONS: LiquidGlassNavOptions = {
  glassThickness: 48,
  bezelWidth: 22,
  ior: 2.35,
  scaleRatio: 0.88,
...
const SURFACE_FNS: Record<LiquidGlassSurface, (x: number) => number> = {
...
export function supportsBackdropFilterUrl(): boolean {
  ...
  if (typeof CSS !== "undefined" && typeof CSS.supports === "function") {
    if (CSS.supports("backdrop-filter", "url(#test)")) return true;
    if (CSS.supports("-webkit-backdrop-filter", "url(#test)")) return true;
  }

  const testEl = document.createElement("div");
  testEl.style.setProperty("backdrop-filter", "url(#test)");
  const value =
    testEl.style.backdropFilter || testEl.style.getPropertyValue("backdrop-filter");
  return value.includes("url(");
}

export function navGlassOptionsForSize(
  width: number,
  height: number,
): LiquidGlassNavOptions {
  const h = Math.max(2, Math.round(height));
  return {
    glassThickness: Math.round(h * 1.15),
    bezelWidth: Math.round(Math.min(h * 0.44, h / 2 - 2)),
    ior: 2.9,
    scaleRatio: 1,
    blurAmount: 1.24,
...
export function buildLiquidGlassFilterMarkup(
  ...
  height: number,
  radius: number,
  options: LiquidGlassNavOptions = DEFAULT_NAV_GLASS_OPTIONS,
): string {
  const w = Math.max(2, Math.round(width));
  const h = Math.max(2, Math.round(height));
  const heightFn = SURFACE_FNS[options.surface];
  ...
  return `
    <filter id="ff-liquid-glass-filter" x="0%" y="0%" width="100%" height="100%">
